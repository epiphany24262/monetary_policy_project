...
《面向经济和金融的 Python 编程》课程研究。考察中国人民银行季度货币政策执行报告中的政策指引文本创新度与金融市场短期反应之间的关系。

## 项目简介

核心检验使用政策指引章节的扩展窗口 TF-IDF 创新度，解释报告发布后五个交易日的股票实际波动率。日度稳健性采用 Student-t EGARCH-X 模型。文本测量包括人工句子标注、语境门控规则、字符 TF-IDF 与 LinearSVC 的按报告分组交叉验证。债券市场部分使用未预期政策语调和跨拟合监督语调，考察国债收益率曲线变化，属于探索性扩展。
...
货币政策报告来自中国人民银行官网。股票指数、国债收益率和政策操作数据来源登记见 `data/source_registry.csv`，许可说明见 `DATA_LICENSE_AND_REDISTRIBUTION.md`。

## 复现注意事项

默认命令使用已生成的本地数据和正式估计结果。日度 EGARCH-X 模型计算量大，如需重新估计，用 `--recompute-heavy`；仅更新学习曲线、跨拟合语调和功效分析，用 `--recompute-text-diagnostics`。
