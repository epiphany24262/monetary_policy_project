...
def _build_body(doc: Document, numbers: PaperNumbers, tables: JournalTables, figures) -> None:
    ...
    for text in [
        ...
        "数据合法性和可追溯性主要体现在来源和口径上。政策报告来自央行公开网页，市场数据保留清洗后的日度序列和来源登记。论文正文不展开文件层面的运行核验说明，但表1给出了每类数据的频率、样本期和用途，读者可以据此判断文本、股票、债券和人工标注样本之间的对应关系。",
    ]:
        _add_body(doc, text)
    ...
    for text in [
        ...
        "未预期语调的构造以报告自身的历史规律为基准，试图区分常规表述和超出惯性的政策倾向变化。这个思路适度参考董青马等（2024）关于“潜在”未预期信息的讨论；具体指标和样本处理均按本文数据重新设定，未采用其完整识别框架。季度报告文本较长，且包含大量宏观判断，未预期语调进入债券回归后仍可能同时反映政策冲击和央行信息冲击。",
    ...
    for text in [
        ...
        "本文只适度参考董青马等（2024）和尚玉皇、刘华和申峰（2025）的研究问题。前者关注资产价格中的潜在未预期政策信息，后者围绕国债收益率曲线建立更系统的预期博弈分析；本文未采用这些研究的高级识别框架，而是在现有数据条件下使用更克制的文本测量和事件回归设计。",
